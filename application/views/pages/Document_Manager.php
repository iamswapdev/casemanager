...
if($data['operation']) {
	...
	try {
		...
		switch($data['operation']) {
			case 'get_node':
				$node = isset($data['id']) && $data['id'] !== '#' ? $data['id'] : '/';
				$rslt = $fs->lst($node, (isset($data['id']) && $data['id'] === '#'));
				break;
			case "get_content":
				$node = isset($data['id']) && $data['id'] !== '#' ? $data['id'] : '/';
				$rslt = $fs->data($node);
...
<html>
	<head>
		...
		<link rel="stylesheet" href="<?php base_url();?>dist/themes/default/style.min.css" />
		<style>
	...
	<body>
		...
		<script src="<?php base_url();?>dist/jstree.min.js"></script>
		<script>
		$(function () {
